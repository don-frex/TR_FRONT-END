  ...
  var pJS = function (e, a) {
	...
	i.fn.retinaInit = function () {
	  i.retina_detect && window.devicePixelRatio > 1
		? ((i.canvas.pxratio = window.devicePixelRatio), (i.tmp.retina = true))
		: ((i.canvas.pxratio = 1), (i.tmp.retina = false))
	...
	i.fn.particle = function (e, a, t) {
	  ...
	  if ('image' == this.shape) {
		...
		'svg' == i.tmp.img_type &&
		  ...
		  i.tmp.pushing && (this.img.loaded = false))
	  }
	}
		  ...
		  (i.particles.opacity.anim.enable &&
			(1 == a.opacity_status
			  ? (a.opacity >= i.particles.opacity.value &&
				  (a.opacity_status = false),
				(a.opacity += a.vo))
			  : (a.opacity <= i.particles.opacity.anim.opacity_min &&
				  (a.opacity_status = true),
				(a.opacity -= a.vo)),
			a.opacity < 0 && (a.opacity = 0)),
		  i.particles.size.anim.enable &&
			(1 == a.size_status
			  ? (a.radius >= i.particles.size.value && (a.size_status = false),
				(a.radius += a.vs))
	...
	i.fn.interact.attractParticles = function (e, a) {
	  ...
	  if (n <= i.particles.line_linked.distance) {
		var r = t / (1000 * i.particles.move.attract.rotateX),
		  c = s / (1000 * i.particles.move.attract.rotateY)
		e.vx -= r
		e.vy -= c
		a.vx += r
		a.vy += c
		...
		if (
		  ...
		  if (i.tmp.bubble_clicking) {
			var s = e.x - i.interactivity.mouse.click_pos_x,
			  ...
			  r = Math.sqrt(s * s + n * n),
			  p = (new Date().getTime() - i.interactivity.mouse.click_time) / 1000
			p > i.interactivity.modes.bubble.duration &&
			  (i.tmp.bubble_duration_end = true)
			p > 2 * i.interactivity.modes.bubble.duration &&
			  ((i.tmp.bubble_clicking = false),
			  (i.tmp.bubble_duration_end = false))
		  }
	...
	i.fn.modes.repulseParticle = function (e) {
	  ...
	  if (
		...
		var t = e.x - i.interactivity.mouse.pos_x,
		  ...
		  l = clamp((1 / c) * (-1 * Math.pow(n / c, 2) + 1) * c * 100, 0, 50),
		  v = {
			x: e.x + r.x * l,
			y: e.y + r.y * l,
		  }
